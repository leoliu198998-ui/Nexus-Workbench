...
## 阶段 1：基础架构与数据库更新
- [ ] Task: 修改 Prisma Schema 以支持 `CANCELLED` 状态
    - [ ] 在 `OutageStatus` 枚举中添加 `CANCELLED`
    - [ ] 执行数据库迁移并生成客户端 (`npx prisma migrate dev --name add_cancelled_status`)
- [ ] Task: 更新 TypeScript 类型定义
    - [ ] 确保 `src/types/outage.ts` 与新的 Prisma 枚举一致
